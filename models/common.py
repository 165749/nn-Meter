...
def flatten(x,
    ...
    # TODO (Zhuojin): Remove unnecessary Transpose operation
    return x
...
class MaxPool2d(nn.Layer):
    ...
    def __init__(self,
        ...
        # TODO (Zhuojin): Use same paddings
        if padding[0] * 2 + 1 == pool_size[0]:
            force_same = True
            self.use_pad = False
        else:
            force_same = False
...
class Conv2d(nn.Layer):
    ...
    def __init__(self,
        ...
        # TODO (zhuojin): Allow same padding
        ...
        if self.use_conv:
            ...
            # TODO (Zhuojin): Update for grouped convolution layer
            self.conv = nn.Conv2D(
                filters=out_channels,
                kernel_size=kernel_size,
                strides=strides,
...
def conv3x3_block(in_channels,
    ...
    data_format : str, default 'channels_last'
        The ordering of the dimensions in tensors.
    """
    force_same = True if padding == 1 else False  # TODO (Zhuojin): For same padding
    return ConvBlock(
        in_channels=in_channels,
        out_channels=out_channels,
        kernel_size=3,
...
def conv5x5_block(in_channels,
    ...
    data_format : str, default 'channels_last'
        The ordering of the dimensions in tensors.
    """
    force_same = True if padding == 2 else False  # TODO (Zhuojin): For same padding
    return ConvBlock(
        in_channels=in_channels,
        out_channels=out_channels,
        kernel_size=5,
...
def conv7x7_block(in_channels,
    ...
    data_format : str, default 'channels_last'
        The ordering of the dimensions in tensors.
    """
    force_same = True if padding == 3 else False  # TODO (Zhuojin): For same padding
    return ConvBlock(
        in_channels=in_channels,
        out_channels=out_channels,
        kernel_size=7,
...
class SEBlock(nn.Layer):
    ...
    def __init__(self,
        ...
        self.pool = nn.GlobalAveragePooling2D(
            keepdims=True,  # TODO (Zhuojin): Avoid use of expand_dims
            data_format=data_format,
            name="pool")
        if use_conv:
            self.conv1 = conv1x1(
    ...
    def call(self, x, training=None):
        w = self.pool(x)
        # TODO (Zhuojin): Avoid use of expand_dims
        # if self.use_conv:
